# -*- coding: utf-8 -*-
"""
Created on Fri Oct  1 11:22:12 2021
...
#dit is voor de accuracy te berekenen
print("Accuracy for ramposition:", accuracy_score(training_predictions_ram, Y).round(3) * 100, "%")
print("accuracy for injection pressure:", accuracy_score(training_predictions_injection, Y).round(3) * 100, "%")
print("Accuracy for sensor pressure:", accuracy_score(training_predictions_pressure, Y).round(3) * 100, "%")
print("Accuracy of the combined training set:", accuracy_score(training_predictions_total, Y).round(3) * 100, "%")

#opslagen in csv
pd.DataFrame(ram_pred, columns=['Ram predictions']).to_csv('ram_pred.csv')
pd.DataFrame(inject_pred, columns=['Inject predictions']).to_csv('inject_pred.csv')
pd.DataFrame(sensor_pred, columns=['Sensor predictions']).to_csv('sensor_pred.csv')
pd.DataFrame(training_predictions_total, columns=['Total predictions']).to_csv('prediction_total.csv')
